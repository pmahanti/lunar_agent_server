...
<div class="wrap">
  ...
  <p class="subtitle">Enter coordinates &rarr; agent chains 6 Python tools &rarr; synthesizes geologic briefing</p>
  ...
  <div class="panels">
    <div class="panel">
      ...
      <div id="toolLog"><p class="empty">Tool calls will appear here&hellip;</p></div>
    </div>
    <div class="panel briefing-panel">
      <div class="panel-label">Geologic briefing</div>
      <div id="briefingOut"><p class="empty">Briefing will appear once tools complete&hellip;</p></div>
    </div>
  </div>
</div>
<script>
...
function makeToolEl(t) {
  ...
  head.appendChild(dot); head.appendChild(name); div.appendChild(head); return div;
}
function setTool(t, status, result) {
  ...
  if(result){ let res=el.querySelector(".tool-result"); if(!res){res=document.createElement("div");res.className="tool-result";el.appendChild(res);} res.textContent=result; }
}
function tags(arr,cls){ return arr.map(f=>'<span class="tag '+(cls||"")+'">'+f+'</span>').join(""); }
async function runAgent() {
  document.getElementById("coordErr").textContent="";
  const agentErrEl=document.getElementById("agentErr"); agentErrEl.style.display="none";
  ...
  const {la,lo}=parsed; lastCoords={la,lo};
  const btn=document.getElementById("runBtn"); btn.disabled=true;
  const toolLog=document.getElementById("toolLog"); toolLog.innerHTML="";
  ...
  let animIdx=0;
  const animInterval=setInterval(()=>{ if(animIdx<TOOLS.length){setTool(TOOLS[animIdx],"running",null);animIdx++;} },300);
  try {
    ...
    for(let i=0;i<TOOLS.length;i++){
      const t=TOOLS[i]; setTool(t,"running",null);
    ...
    agentErrEl.textContent="Agent error: "+err.message; agentErrEl.style.display="block";
